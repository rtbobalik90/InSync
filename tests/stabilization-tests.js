...
// 10d2. Expedition sync carries route/leg identity even when health mileage is private.
{
  ...
  let p=C.sharePayload();
  eq(p.schema,8,'partner payload uses current sync schema 8');
...
// 12. Static regression checks around photos, backups, navigation, cache and assets.
{
  ...
  ok(sw.includes("CACHE = 'insync-v10-33'") && sw.includes('e.waitUntil(fresh'),'service worker uses the refreshed v10 cache and stale-while-revalidate artwork');
